...
en utilisant la syntaxe
  interface NOM extends TYPE (array ou map)
on pourrait rendre le code plus lisible.

Array et map permettent l'utilisation de la syntaxe "for object of objects"
qui donne accès directement à l'objet en ne passant pas par la syntaxe objects[object]
Map permet quand à lui de faire des "for [key, value] of objects"
et d'accéder également directement à key et value

*/

type resourceTypeDictionary = Record<string, { arrayIndex: number }>
...
type ReplacementArray = ReplacementItem[]

interface OrderedLocalItem {
  value: string
  variable: string
}

export class TerraformResources implements vscode.TreeDataProvider<TfObject> {
  projectObjects: Promise<TfObject[]>
  projectLocals: LocalArray | undefined
  projectResources: ResourceArray | undefined
  context: vscode.ExtensionContext
  activeEditor: vscode.TextEditor | undefined
  maxLevel: number | undefined
  terraformPath: string | undefined
  ...
  private readonly _onDidChangeTreeData: vscode.EventEmitter<TfObject | undefined | null | void> = new vscode.EventEmitter<TfObject | undefined | null | void>()
  readonly onDidChangeTreeData: vscode.Event<TfObject | undefined | null | void> = this._onDidChangeTreeData.event
  ...
  getTreeItem (element: TfObject): vscode.TreeItem {
    return element
  }
   ...
   *
   */
  private async getAllObjects (): Promise<TfObject[]> {
   ...
   */
  private async evalTerraformBaseDirectory (): Promise<TfObject[]> {
    let tfRoot = ''
    let tfRootDepth = 100
    ...
    if (!tfRoot.endsWith('/')) { tfRoot = tfRoot + '/' }
  ...
  private async getlocals (): Promise<TfObject[]> {
    const matchRecursive = require('./match-recursive-improved.js') // eslint-disable-line @typescript-eslint/no-var-requires

    const localObjects: TfObject[] = []
    const allLocals: LocalArray = {} // all_locals est un dictionnaire qui contient tous les locals de tous les fichiers
    ...
    const workspace: string | undefined = this.context.workspaceState.get('activeWorkspace')
    ...
    while (toEval) {
      ...
      for (const localName in allLocals) {
        ...
        if (allLocals[localName].evalcomplete) { continue }
        ...
        if (checkReplacement !== null) {
          ...
          if (checkReplacement[2] === 'terraform') {
            ...
            if (allLocals[checkReplacement[3]] === undefined) { continue } // missing stuff => you'll know it when running validate ^^
          ...
          if (!evalComplete) { toEval = true; continue } // Skip this replacement if eval is not complete
          allLocals[localName].eval = allLocals[localName].eval.replace(searchString, replaceString)
          allLocals[localName].level += 1
          if (allLocals[localName].level > this.maxLevel) { this.maxLevel = allLocals[localName].level } // setting a maxLevel for value->variable replacement
          ...
          toEval = true // in order to use the newly evaluated variable
        } else { allLocals[localName].evalcomplete = true }
        if (iteration > 15) { toEval = false } // protection mechanism => Allow only 10 full replacement cycles to prevent process hang
      }
    }

    // Transforme le dictionnaire en tableau d'objets
    for (const localName in allLocals) {
      if (localName === 'terraform.workspace') { continue }
    ...
    const localRoot: TfObject = new TfObject('Locals', 'Root', vscode.TreeItemCollapsibleState.Expanded, undefined, undefined, undefined)
  ...
  private async getResources (): Promise<TfObject[]> {
    const resourcesDictionary: ResourceArray = {}
    ...
    for (const resourceId in resourcesDictionary) {
      ...
      if (!resourceTypeDict[resourceType]) {
        ...
        const anyRoot: TfObject = new TfObject(resourceType, 'Root', vscode.TreeItemCollapsibleState.Collapsed, undefined, undefined, undefined)
        returnArray.push(anyRoot)
        idx++
      }

      const object = new TfObject(resource.name,
        resource.subtype ?? resourceType,
        resource.subtype && resource.subtype === 'View' ? vscode.TreeItemCollapsibleState.Collapsed : vscode.TreeItemCollapsibleState.None,
        undefined,
        resource.uri,
        resource.strpos)
  ...
  private resourceAnalysis (resourceList: Record<string, any>, uri: vscode.Uri, rawFile: string): ResourceArray {
    const resourcesDictionary: ResourceArray = {}
    let stringPos: number
    ...
    for (const resourceType in resourceList) {
      ...
      for (const resourceName in bqtableResources) {
        ...
        const vars: Record<string, { eval: string }> = {}
        let schemaPath: string | undefined
        let disabledFlag: boolean = false
        let tableId: string | undefined
        let tablePartition: string | undefined
        ...
        if (resourceType === 'google_bigquery_table') {
          ...
          if (resourceDeclaration.view) {
            subtype = 'View'
            const view: string = resourceDeclaration.view[0].query
  ...
  public registerCommands (): void {
    ...
    const bqHelperGenDrawIo = vscode.commands.registerCommand('bigqueryhelper.genDrawIo', (item: TfObject) => {
      void this.genRelationalSchema(item, 'createTable', 'Table')
    })
    this.context.subscriptions.push(bqHelperGenDrawIo)

    const bqHelperGenPlantUml = vscode.commands.registerCommand('bigqueryhelper.genPlantUml', (item: TfObject) => {
      void this.genRelationalSchema(item, 'plantUml', 'Table')
    })
    this.context.subscriptions.push(bqHelperGenPlantUml)

    const bqHelperGenPlantUmlWViews = vscode.commands.registerCommand('bigqueryhelper.genPlantUmlWViews', (item: TfObject) => {
    ...
    const sqlTFReplacedHandler = (sqlText: string, filename: string): string => {
      const workspace: string | undefined = this.context.workspaceState.get('activeWorkspace')
      let query = ''
      if (workspace) {
    ...
    const moveSqlToFile = vscode.commands.registerCommand('bigqueryhelper.moveSqlToFile', (item: TfObject) => {
      void this.prepareMoveViewSchema('SQL')
    })
    this.context.subscriptions.push(moveSqlToFile)

    const moveSchemaToFile = vscode.commands.registerCommand('bigqueryhelper.moveSchemaToFile', (item: TfObject) => {
      void this.prepareMoveViewSchema('SCHEMA')
    })
    this.context.subscriptions.push(moveSchemaToFile)
  }

  private buildOrderedLocalsArray (localsList: LocalArray): OrderedLocalItem[] {
    // on pourrait retourner le tableau non ordonné ?
    const returnArray: OrderedLocalItem[] = []
    if (!this.maxLevel) {
    ...
    let currentLevel: number = this.maxLevel
  ...
  private async terraformReplace (direction: string): Promise<void> {
    ...
    if (document && workspace !== undefined) {
      ...
      if (fileContent !== undefined) {
        ...
        const textRangeArray: [{ range: vscode.Range, text: string }?] = []
  ...
  private getReplacements (type: string, direction: string, filename?: string): ReplacementArray {
    const replacementArray: ReplacementArray = []
    const terraformWorkspace: string | undefined = this.context.workspaceState.get('activeWorkspace')
      ...
      for (const resourceId in this.projectResources) {
        ...
        if (resource.subtype === 'View' && resource.templatefile && filename.toLowerCase().endsWith(resource.templatefile.toLowerCase()) && resource.variables) {
          for (const variable in resource.variables) {
            ...
            } else { if (baseValue) { replacementArray.push({ string: baseValue, replacement: '${' + variable + '}' }) } }
          }
        }
      }
    }
      ...
      for (const resourceId in this.projectResources) {
        ...
        if (resource.subtype === 'View' && resource.templatefile && filename.toLowerCase().endsWith(resource.templatefile.toLowerCase()) && resource.variables) {
          for (const variable in resource.variables) {
            ...
            if (resource.variables[variable].eval === 'terraform.workspace') { value = terraformWorkspace } else {
            ...
            if (value) { replacementArray.push({ string: '${' + variable + '}', replacement: value }) }
          }
        }
      }
    }
        ...
        for (const local of localsArray) {
          ...
          if (local.value.includes('${terraform.workspace}')) {
            for (const item in terraformWorkspacesList) { /* TODO See if we can fix this es-lint error */ // eslint-disable-line @typescript-eslint/no-for-in-array
              ...
              if (value) { replacementArray.push({ string: value, replacement: variable }) };
            }
          } else { replacementArray.push({ string: local.value, replacement: variable }) }
        }
      }
    }
  ...
  private replaceLocals (text: string, fileName: string, workspace: string, type: string, direction: string): string {
    const replacements: ReplacementArray = this.getReplacements(type, direction, fileName.replaceAll('\\', '/'))
  ...
  private async applyLocalsReplacement (replacement: string, range: vscode.Range): Promise<void> {
  ...
  private stripComments (baseString: string): string {
  ...
  private gotoResource (item: TfObject): void {
    if (item.uri) {
      void vscode.commands.executeCommand('vscode.open', item.uri).then((any: any) => {
  ...
  public async getSchema (schemaPath: vscode.Uri): Promise<Column[] | null | undefined> {
...
*/
  ...
  private async genRelationalSchema (item: TfObject, type: string, scope: string): Promise<void> {
    let createTable: string = ''
    let plantUml: string = ''
    let links: string = ''
    const plantUmlDataArray: PlantUmlData = {}
    const bqtfConfig = vscode.workspace.getConfiguration('bqtf')
    const plantUmlconfig: string | undefined = bqtfConfig.get('plantUmlConfig')
    ...
    const ressourceToHandle: ResourceArray = {}
    ...
    for (const resource in ressourceToHandle) {
      ...
      if (resourceObject.schemaPath && !resourceObject.disabledFlag) {
        ...
        if (schemaPath && tableName) {
          ...
          let partitionFlag: boolean = false
          let clusteringFlag: boolean = false
          if (resourceObject.table_partition) {
            if (plantUmlDataArray[tableName] === undefined) { plantUmlDataArray[tableName] = {} }
   ...
   */
  private async findRelatedTable (view: string, resources: ResourceArray): Promise<string | null> {
  ...
  private async genCreateTable (schema: Column[], tableName: string): Promise<string> {
    ...
    for (const column of schema) {
      ...
      if (column.description?.match(/\[[U]*PK[1-9]*\]/m) !== null) { createTable += ' PRIMARY KEY' }
      createTable += closure
  ...
  private async genPlantUml (schema: Column[], tableName: string, type: string, partitionFlag: boolean, clusteringFlag: boolean): Promise<PlantUmlResponse> {
    // At the moment, can't handle both PK and FK + un seule FK
    let plantUml = 'table( ' + tableName + ' ) {\r\n'
    if (type === 'View') { plantUml = 'view( ' + tableName + ' ) {\r\n' }
    const fkLink: string[] = []
    const linkDefinitions: string[] = []
    if (partitionFlag) { plantUml += '  part("' + tableName + '") \r\n' }
    if (clusteringFlag) { plantUml += '  cluster("' + tableName + '") \r\n' }
    if (type === 'Table') { plantUml += '  ---\r\n' }
    for (const column of schema) {
      let topPk = false
      let topFk = false
  ...
  public getSchemaPath (path: string): vscode.Uri | undefined {
  ...
  */
  private async prepareMoveViewSchema (type: string): Promise<any> {
    ...
    void vscode.window.showOpenDialog({ canSelectFiles: false, canSelectFolders: true, canSelectMany: false, defaultUri: basePath }).then((uri: vscode.Uri[] | undefined) => {
      if (uri) {
  ...
  private async moveViewSchemaToFile (targetPath: vscode.Uri, type: string): Promise<void> {
    if (vscode.workspace.workspaceFolders === undefined || this.terraformPath === undefined) {
      await Promise.reject(new Error('No workspace found')); return
    }
  ...
  private getTerraformBasePath (): string | undefined {
    return this.terraformPath
  }

  private getLocalsList (): LocalArray | undefined {
    return this.projectLocals
  }

  private getResourcesList (): ResourceArray | undefined {
...
class TfObject extends vscode.TreeItem {
  constructor (
    public readonly objectName: string, // The name
    public type: string, // local/ table / view / dataset
    public readonly collapsibleState?: vscode.TreeItemCollapsibleState,
    public children?: TfObject[], // List of childrens
    public uri?: vscode.Uri,
    public strpos?: number // string position
  ...
  /**
   addChildren
   */
  public addChild (child: TfObject): void {
  ...
  public addChildren (childArray: TfObject[]): void {
    this.children = childArray
  }

  public getChildren (): TfObject[] | undefined {
